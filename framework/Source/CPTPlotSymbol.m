 ...
 *  @brief The drawing text for a custom plot symbol. It will be scaled to @ref size before being drawn.
 **/
@synthesize text;
/** @property CPTColor textColor
 *  @brief The drawing text color for a custom plot symbol. It will be scaled to @ref size before being drawn.
 **/
@synthesize textColor;

@synthesize textOffsetPoint;
    ...
    if ( theSymbolPath ) {
        ...
        if ( theLineStyle || theFill ) {
            ...
            if (self.text) {
                ...
                UIFont * font = [UIFont systemFontOfSize:(self.fontSize *scale)];
                CGSize fontBoundSize = [self.text sizeWithFont:font];
#if TARGET_IPHONE_SIMULATOR || TARGET_OS_IPHONE
                CGContextSaveGState(context);
                CGContextTranslateCTM(context, CPTFloat(0.0), center.y - fontBoundSize.height/2);
                CGContextScaleCTM( context, CPTFloat(1.0), CPTFloat(-1.0) );
                CGContextTranslateCTM( context, CPTFloat(0.0), -(center.y + fontBoundSize.height/2));
#endif
                [self.textColor set];
                [self.text drawAtPoint:CGPointMake(center.x - fontBoundSize.width/2 + self.textOffsetPoint.width, center.y - fontBoundSize.height/2 + self.textOffsetPoint.height)
                         forWidth:fontBoundSize.width
                         withFont:font
